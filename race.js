...
Number.prototype.pad = function(numZeros, char = 0) {
    let n = Math.abs(this)
    let zeros = Math.max(0, numZeros - Math.floor(n).toString().lenth)
...
function drawQuad(element, layer, color, x1, y1, w1, x2, y2, w2) {
    ...
    element.style.clinpPath = `polygon($(leftOffset)px 0, $(leftOffset + w2)px 0, 66.6% 100%, 33.3% 100%)`
}
const KEYS = {}
const keyUpdate = e => {
...
function update(step) {
    ...
    if(!inGame) {
        ...
        timestamp.innerText = (countDown|0).pad(3)
        scoreVal.innerText = (scoreVal|0).pad(8)
